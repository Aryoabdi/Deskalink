...
$result = $conn->query("SELECT * FROM designs WHERE partner_id = '$partner_id' ORDER BY created_at DESC");
?>
<!DOCTYPE html>
<html lang="id">
<head>
    ...
    <title>Desain Digital Saya</title>
...
<main class="flex-1 p-6 ml-0 md:ml-64">
    <div class="container mx-auto px-4">
        <h1 class="text-3xl font-bold mb-6">Desain Digital Saya</h1>

        <!-- Tombol Tambah Desain -->
        <a href="add_design.php" class="bg-green-600 px-4 py-2 rounded hover:bg-green-700 mb-4 inline-block">
            + Unggah Desain Digital
        </a>
        ...
        <div class="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            <div class="w-full max-w-xs bg-gray-800 rounded shadow p-4">
                <?php if ($row['thumbnail']) : ?>
                    <img src="<?= $row['thumbnail'] ?>" alt="Thumbnail" class="w-full h-40 object-cover rounded mt-2">
                <?php endif; ?>
                ...
                <div class="flex gap-2 mt-3">
                    <a href="edit_design.php?id=<?= $row['design_id'] ?>" class="bg-blue-500 px-3 py-1 rounded text-sm">Edit</a>
                    <button onclick="deleteDesign(<?= $row['design_id'] ?>)" class="bg-red-500 px-3 py-1 rounded text-sm">Hapus</button>
                </div>
            </div>
            <?php endwhile; ?>
        </div>
...
function deleteDesign(id) {
    if (confirm("Yakin ingin menghapus desain ini?")) {
        $.post("delete_design.php", { id }, function(res) {
            alert(res);
            location.reload();
        });
    }
